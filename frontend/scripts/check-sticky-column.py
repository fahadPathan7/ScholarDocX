...
# confirms that defence is present rather than trying to police every rule
# that could otherwise override it.
chrome = open("src/components/sheet/sheet-chrome.css").read()
defended = all(
    re.search(rf'\.sheet-table td\.row-header\s*\{{[^}}]*{p}\s*:[^;}}]*!important', chrome, re.S)
    for p in ("background", "box-shadow")
)
if not defended:
    print("STICKY COLUMN GUARD FAILED:")
    print("  .sheet-table td.row-header must set background AND box-shadow with")
    print("  !important in sheet-chrome.css, or a `... td` rule elsewhere will")
    print("  strip them and the column will render see-through.")
    sys.exit(1)
