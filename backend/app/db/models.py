...
class Users(Base):
    ...
    research_papers: Mapped[list['ResearchPapers']] = relationship('ResearchPapers', back_populates='user', cascade='all, delete-orphan')
...
class ResearchPapers(Base):
    """Uploaded research papers for the Research Expert workspace (SCHOLARDOCX-0174).

    Each row represents one user-uploaded PDF. The extracted full text is stored
    for re-chunking or display purposes; individual chunks live in
    ``research_paper_chunks``. Foreign key to ``static_files`` links to the
    Supabase-stored PDF binary; CASCADE delete on both user_id and
    static_file_id ensures cleanup propagates.
    """
    __tablename__ = 'research_papers'
    __table_args__ = (
        Index('idx_research_papers_user_id', 'user_id'),
    )
    ...
    publication_year: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    volume_issue_pages: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    doi: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    created_at: Mapped[str] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())
    updated_at: Mapped[str] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()), server_default=text("gen_random_uuid()"))

    user: Mapped['Users'] = relationship('Users', back_populates='research_papers')
    chunks: Mapped[list['ResearchPaperChunks']] = relationship('ResearchPaperChunks', back_populates='paper', cascade='all, delete-orphan')


class ResearchPaperChunks(Base):
    """Text chunks with vector embeddings for Research Expert (SCHOLARDOCX-0174).

    Each chunk is a semantic segment (~500-1000 tokens) of a research paper.
    The ``embedding`` column stores a 1024-dimension float vector generated by
    Jina AI jina-embeddings-v4. pgvector's cosine distance operator (``<=>``) is
    used for similarity search. The HNSW index accelerates nearest-neighbour
    queries once the table has enough rows (pgvector recommends > 1000).
    
    Note: 1024 dimensions chosen to stay under pgvector's 2000-dim limit for indexes.
    """
    __tablename__ = 'research_paper_chunks'
    __table_args__ = (
        Index('idx_research_paper_chunks_paper_id', 'paper_id'),
    )

    paper_id: Mapped[str] = mapped_column(ForeignKey('research_papers.id', ondelete='CASCADE'), nullable=False)
    ...
    paper: Mapped['ResearchPapers'] = relationship('ResearchPapers', back_populates='chunks')


class ResearchPaperAnalyses(Base):
    """Saved AI analysis outputs for a Research Expert paper (SCHOLARDOCX-0174).

    Users can persist up to 10 analysis results per paper so they can revisit an
    answer later without re-running (and re-paying for) the query. ``sources_json``
    stores the citation/source array as a JSON string. Rows are removed via the
    database-level ``ON DELETE CASCADE`` when the parent paper (or user) is
    deleted, so no application-side cleanup is required.
    """
    ...
    charged_credits: Mapped[int] = mapped_column(Integer, nullable=False, server_default=text('0'))
    created_at: Mapped[str] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()), server_default=text("gen_random_uuid()"))
