...
# SCHOLARDOCX-0178: fixed cap on saved Opportunity Library entries per user.
# Not an admin-configurable role limit — a static ceiling, documented as
# information in the Admin panel's External APIs modal.
MAX_LIBRARY_ENTRIES = 100


class LibraryFullError(Exception):
    """Raised when a user's Opportunity Library is at MAX_LIBRARY_ENTRIES and
    a NEW opportunity (not an update to an already-saved URL) is attempted."""
...
async def get_scholarship_catalog(
    ...
    category: Optional[str] = Query(None, description="Filter by 'program' or 'university'"),
    user: dict = Depends(get_current_user),
    store: Store = Depends(get_user_store),
) -> List[Dict[str, Any]]:
    """List the static scholarship catalog (SCHOLARDOCX-0176: static-only).

    No network calls, no billing. The paid 'Check current cycle' action was
    removed; the catalog is a curated reference with official links only.
    """
...
def upsert_scholarship_opportunity(
    ...
    *,
    ...
    existing_rows = store.list_records("scholarship_opportunities")
    existing = next(
        (row for row in existing_rows if row["normalized_url"] == normalized),
        None,
    )
    # SCHOLARDOCX-0178: the cap only blocks growth (a brand-new saved
    # opportunity). Updating a URL the user already saved (re-analyzing,
    # deep-hunt re-finding the same page) is never blocked by it.
    if not existing and len(existing_rows) >= MAX_LIBRARY_ENTRIES:
        raise LibraryFullError(
            f"Your Opportunity Library is full (max {MAX_LIBRARY_ENTRIES}). "
            "Remove an existing opportunity before saving a new one."
        )
    if existing:
